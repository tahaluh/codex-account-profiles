...
function formatLimitHtml(result: RateLimits): string {
  ...
  if (nextReset) summaryParts.push(`Next reset ${new Date(nextReset * 1000).toLocaleString()}`);
...
class AccountsView implements vscode.WebviewViewProvider {
  ...
  async refresh(force = false): Promise<void> {
    ...
    try {
      const current = JSON.parse(await fs.readFile(path.join(this.context.globalStorageUri.fsPath, "current-account.json"), "utf8"));
